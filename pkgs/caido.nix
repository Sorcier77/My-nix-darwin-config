...
let
  ...
  desktopSource = desktopSources.${system} or (throw "Unsupported system for caido-desktop: ${system}");

  cli = fetchurl { url = cliSource.url; hash = cliSource.hash; };
  desktop = fetchurl { url = desktopSource.url; hash = desktopSource.hash; };

  appimageContents = appimageTools.extractType2 { inherit pname version; src = desktop; };
  ...
  wrappedDesktop =
    ...
    # ← BLOC DARWIN FIXÉ COMME OBSIDIAN
    else if isDarwin then
      stdenv.mkDerivation {
        src = desktop;
        inherit pname version;

        sourceRoot = "Caido.app";  # ← comme obsidian: sourceRoot = "${appname}.app"
        ...
        installPhase = ''
          runHook preInstall
          mkdir -p $out/{Applications/Caido.app,bin}
          cp -R . $out/Applications/Caido.app
          makeWrapper $out/Applications/Caido.app/Contents/MacOS/Caido $out/bin/caido
          runHook postInstall
        '';
  ...
  meta = {
    ...
    maintainers = with lib.maintainers; [ octodi d3vil0p3r blackzeshi ];
    platforms = [ "x86_64-linux" "aarch64-linux" "x86_64-darwin" "aarch64-darwin" ];
  };
in
lib.checkListOfEnum "${pname}: appVariants" appVariantList appVariants (
  if appVariants == [ "desktop" ] then wrappedDesktop
  else if appVariants == [ "cli" ] then wrappedCli
  else
    stdenv.mkDerivation {
      ...
      installPhase = ''
        ...
        ln -s ${wrappedDesktop}/Applications $out/Applications
      '';
    }
)
